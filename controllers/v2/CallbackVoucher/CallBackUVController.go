...
type V2_CallbackUVController struct{}

func (controller *V2_CallbackUVController) CallBackUVController(ctx *gin.Context) {

	logrus.Info("[ >>>>>>>>>>>>>>>>>>>>> V2 Migrate Callback Voucher UV COntroller <<<<<<<<<<<<<<<<<< ]")
	...
	CallabckUVServices := Redeemtion.V2_VoucherUVService{
