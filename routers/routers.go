...
func Server(portStr string) error {
	...
	apiRoot := router.Group("/transaction")
	{
		v2Root := apiRoot.Group("/v2")
		{
			...
			v2Root.GET("/usevoucher", use_vouchers.UseVouchersControllers)
			v2Root.GET("/earningpoint", earning.EarningsPointController)
			v2Root.GET("/check-status-earning", earning.CheckStatusEarningController)
			...
			v2Root.GET("/redeempoint", redeem.RedeemtionControllerV21)
			v2Root.GET("/status/sepulsa", callback_v21.CallBackSepulsa_V21_Controller)
			v2Root.GET("/redeem/voucherag", callbacks.CallbackVoucherAggController)
			v2Root.GET("/usevoucher_uv", callbacks.CallBackUVController)
		}
