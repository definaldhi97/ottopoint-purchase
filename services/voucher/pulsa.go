...
func RedeemPulsa(req models.UseRedeemRequest, reqOP interface{}, param models.Params) models.UseRedeemResponse {
	...
	logrus.Info("[Start]-[Package-Voucher]-[RedeemPulsa]")

	// Validasi Prefix
	dataPrefix, errPrefix := db.GetOperatorCodebyPrefix(req.CustID)
	if errPrefix != nil {

		logrus.Info("[ErrorPrefix]-[services-RedeemPulsa]")
		logrus.Info(fmt.Sprintf("dataPrefix = %v", dataPrefix))
		logrus.Info(fmt.Sprintf("Prefix tidak ditemukan %v", errPrefix))
	...
	if inqRespOttoag.Rc != "00" {
		logrus.Info("[Error-InquiryResponse]-[RedeemPulsa]")
...
func ValidatePrefix(OperatorCode int, custID, productCode string) bool {
	...
	logrus.Info("===== Req.Product =====", productCode)
	// no, _ := strconv.Atoi(custID)
	logrus.Info("===== NO =====", custID)
	prefix := utils.Operator(OperatorCode)
	logrus.Info("===== Prefix =====", prefix)
	product := utils.ProductPulsa(productCode[0:4])
	logrus.Info("===== Product =====", product)

	logrus.Info("===== CustID =====", custID)
	if len(custID) < 4 {

		logrus.Info("[FAILED]-[Prefix-ottopoint]-[services-RedeemPulsa]")
		logrus.Info(fmt.Sprintf("invalid Prefix %s", custID))
	...
	if prefix != product {
		...
		logrus.Info("[FAILED]-[Prefix-ottopoint]-[services-RedeemPulsa]")
		logrus.Info(fmt.Sprintf("invalid Prefix %s", prefix))
...
func SaveTransactionPulsa(param models.Params, res interface{}, reqdata interface{}, reqOP interface{}, trasnType, status, rc string) string {
	...
	logrus.Info("[Start-SaveDB]-[Pulsa]")
