...
var (
	host                      = ODU.GetEnv("OTTOPOINT_PURCHASE", "http://13.228.25.85:8006")
	callbackOttoPointPurchase = ODU.GetEnv("ENDPOINT_CALLBACK_VOUCHERAG", "/transaction/v2/callback/redeem/voucherag")
)
...
func (t VoucherAgMigrateServices) VoucherAgg(req models.VoucherComultaiveReq, param models.Params, header models.RequestHeader) models.Response {
	...
	RedeemVouchAG, errRedeemVouchAG := Redeem_PointandVoucher(req.Jumlah, param, param.CumReffnum, header)

	if RedeemVouchAG.Rc == "10" || RedeemVouchAG.Rd == "Insufficient Point" {

		logrus.Info("[VoucherAgregatorServices]-[RedeemVoucher]")
	...
	if RedeemVouchAG.Rc == "208" || RedeemVouchAG.Rd == "Voucher not available" {
	...
	// var c string
	// for _, vall := range RedeemVouchAG.CouponseVouch {
	// 	c = vall.CouponsCode
	// }
	...
	reqOrder := vgmodels.RequestOrderVoucherAg{
		...
		DeliveryMethod: 1,
		RedeemCallback: host + callbackOttoPointPurchase,
	}
	...
	// Generate Signature
	sign, err := signature.Signature(reqOrder, header)
	if err != nil {
		res = models.Response{
			Meta: utils.ResponseMetaOK(),
	...
	for _, k := range s.MapKeys() {
		header.Signature = fmt.Sprintf("%s", s.MapIndex(k))
	}

	order, errorder := vg.OrderVoucher(reqOrder, header)
	...
	if errorder != nil || order.ResponseCode == "" {
		...
		for i := req.Jumlah; i > 0; i-- {
			...
			go SaveTransactionVoucherAgMigrate(param, order, reqOrder, req, constants.CODE_TRANSTYPE_REDEMPTION, "09")
		}
		res = models.Response{
			Meta: utils.ResponseMetaOK(),
	...
	if order.ResponseCode == "04" {
		...
		for i := req.Jumlah; i > 0; i-- {
			...
			go SaveTransactionVoucherAgMigrate(param, order, reqOrder, req, constants.CODE_TRANSTYPE_REDEMPTION, "01")
		}
	...
	if errStatus != nil {
		...
		logrus.Info("[VoucherAgServices]-[FailedCheckOrderStatus]")
		logrus.Error("[Failed Check Order Status]-[Gagal Order Voucher]")
	...
	for i := req.Jumlah; i > 0; i-- {
		...
		go SaveTransactionVoucherAgMigrate(param, order, reqOrder, req, constants.CODE_TRANSTYPE_REDEMPTION, "00")
...
func (t VoucherAgMigrateServices) CallbackVoucherAgg(req models.CallbackRequestVoucherAg) models.Response {

	fmt.Println("[ >>>>>>>>>>>>>>>>>>>>> Callbakc Voucher Agg Service <<<<<<<<<<<<<<<<<< ]")
	var res models.Response
...
func SaveTransactionVoucherAgMigrate(param models.Params, res interface{}, reqdata interface{}, reqOP interface{}, transType, status string) {
