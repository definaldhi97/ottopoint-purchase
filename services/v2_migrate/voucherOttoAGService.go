...
func (t VoucherOttoAgMigrateService) VoucherOttoAg(req models.VoucherComultaiveReq, param models.Params, header models.RequestHeader) models.Response {
	...
	for i := 0; i < req.Jumlah; i++ {
		...
		go RedeemVoucherOttoAg(req, param, header, getRespChan, getErrChan)
		if getErr := <-getErrChan; getErr != nil {
			getResp = <-getRespChan

			logrus.Info("[ Failed Deduct point, Deduct voucher or Inquiry Voucher ]")
			logrus.Error("Error Message : ", getResp.Message)
			inqGagal++
			continue
		} else {

			logrus.Info("[ Success Deduct point, Deduct voucher and Inquiry Voucher")
			getResp = <-getRespChan
		}

		logrus.Info("[ Response Code RedeemVoucherOttoAg : ", getResp.Code)
		if getResp.Code == "00" {
			wg.Add(1)
	...
	logrus.Info("[ Response OttoAG Payment ] : ", getResRedeem)
	...
	logrus.Info("[ Get RC Payment T_Spending by TSP02 ] : ", rcUseVoucher)
	...
	* -----------------------------------------------
	...
	logrus.Info(" jumlah transaction payment : ", countPayment.Count)
	logrus.Info(" jumlah success transaction success : ", countSuccess.Count)
	logrus.Info(" jumlah success transaction Pending : ", countPending.Count)
	logrus.Info(" jumlah success transaction failed : ", pyenmentFail)
	logrus.Info(" jumlah request : ", req.Jumlah)
	logrus.Info(" Category : ", param.Category)
