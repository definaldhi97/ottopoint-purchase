...
func (t EarningServices) EarningPointSupplyChen(req models.RulePointReq, dataToken redismodels.TokenResp, header models.RequestHeader) models.Response {
	res := models.Response{}
	...
	span, _ := opentracing.StartSpanFromContext(t.General.Context, "[EarningPointSupplyChen-Services]")
	defer span.Finish()
	...
	dataConf, errConf := db.GetConfig()
	if errConf != nil {
		sugarLogger.Info("[EEROR-DATABASE]-[EarningPointSupplyChen-Services]-[GetConfig-DB]")
		sugarLogger.Info(fmt.Sprintf("Failed Get Data from DB getConfig %v", errConf))

		logs.Info("[EEROR-DATABASE]-[EarningPointSupplyChen-Services]-[GetConfig-DB]")
		logs.Info(fmt.Sprintf("Failed Get Data from DB getConfig %v", errConf))
	...
	// perhitungan point
	point := float64(req.Amount) * dataConf.TransaksiMerchant
	// Ngambil angka di depan koma
	point = math.Floor(point)
	totalPoint := fmt.Sprintf("%.f", point)
